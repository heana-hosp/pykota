...
#
# $Id: lidil.py 3578 2019-02-14 08:46:15Z jerome $
#
...
class Parser(pdlparser.PDLParser):
    ...
    def is_valid(self):
        ...
        bof_marker = b"$\x01\x00\x00\x07"
        # End Of File marker is a Sync Complete packet followed
        # with a Reset packet. We ignore the preceding Sync packet
        # for simplicity's sake.
        eof_marker = b"$\x00\x10\x00\x08\x00\x00\x00\x00\x00\xff\xff\xff" \
                     b"\xff\xff$$\x00\x10\x00\x06\x00\x00\x00\x00\x00\xff" \
                    b"\xff\xff\xff\xff$"
        if self.firstblock.startswith(bof_marker) \
           and self.lastblock.endswith(eof_marker):
            return True
        else:
            return False
