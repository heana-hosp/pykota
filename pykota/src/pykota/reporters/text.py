    ...
    def generate_report(self):
        ...
        for printer in self.printers:
            ...
            for (entry, entrypquota) in getattr(self.tool.storage, "getPrinter%ssAndQuotas" % prefix)(printer,
                ...
                self.report.append("%-15.15s %s %5s %7i %7s %7s %10s %-10.10s %8i %10s %4s" % (
                name, reached, overcharge, pagecounter, soft, hard, balance, datelimit, lifepagecounter, lifetimepaid,
                warncount))
                total += pages
                totalmoney += money
            ...
            self.report.append("")
        if self.isgroup:
            self.report.append("Totals may be inaccurate if some users are members of several groups.")
        return "\n".join(self.report)
