...
try:
    ...
    raise PyKotaStorageError(
        f"This python version ({sys.version.split()[0]}) doesn't seem to have the MySQL module installed correctly.")
...
class Storage(BaseStorage, SQLStorage):
    def __init__(self, pykotatool, host, dbname, user, passwd):
        ...
        try:
            ...
            raise PyKotaStorageError("Your version of python-mysqldb is too old. Please install a newer release.")
        self.cursor = self.database.cursor()
        self.cursor.execute("SET NAMES 'utf8';")
        self.cursor.execute(
            "SET TRANSACTION ISOLATION LEVEL READ COMMITTED;")  # Same as PostgreSQL and Oracle's default
        self.closed = 0
