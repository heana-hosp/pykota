...
class IPPRequest:
    ...
    def __str__(self):
        ...
        for attrtype in self.attributes_types:
            attrdict = getattr(self, f"{attrtype}_attributes")
            if attrdict:
                mybuffer.append(f"{attrtype.title()} attributes :")
                for key in attrdict.keys():
                    key_rpl = str(key).replace("b'", "").replace("'", "")
                    key_val = str(attrdict[key]).replace("b'", "'")
                    mybuffer.append(f"  {key_rpl} : {key_val}")
        if self.data:
    ...
    def parse(self):
        ...
        self._curname = None
        self._curdict = None
        self.version = (self._data[0], self._data[1])
    ...
    def parse_tag(self):
        ...
        pos = self.position
        tagtype = self.tags[int(self._data[pos])]
        pos += 1
        posend = pos2 = pos + 2
