...
def get(application, varname) :
    ...
    try :
        value = getattr(application.config, "get{varname}".format(**locals()))(pname)
        if value is None :
        ...
        application.logdebug("No value defined for {varname} for printer {pname}, using {value}.".format(**locals()))
    return value    
