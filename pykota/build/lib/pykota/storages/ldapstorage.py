...
try:
    ...
    raise PyKotaStorageError(
        f"This python version ({sys.version.split()[0]}) doesn't seem to have the python-ldap module installed correctly.")
else:
    try:
        ...
        sys.stderr.write(
            "ERROR: PyKota requires a newer version of python-ldap. Workaround activated. Please upgrade python-ldap !\n")
...
class Storage(BaseStorage):
    ...
    def secondStageInit(self):
        ...
        self.info = self.tool.config.get_ldap_info()
        message = ""
        for tryit in range(3):
            try:
                self.tool.logdebug(
                    f"Trying to open database (host={self.savedhost}, dbname={self.saveddbname}, user={self.saveduser})...")
                ...
                self.tool.printInfo(f"{message}", "error")
                self.tool.printInfo("Trying again in 2 seconds...", "warn")
                time.sleep(2)
            except ldap.LDAPError:
                message = f"Unable to connect to LDAP server {self.savedhost} as {self.saveduser}."
                self.tool.printInfo(f"{message}", "error")
                self.tool.printInfo("Trying again in 2 seconds...", "warn")
                time.sleep(2)
            else:
                self.useldapcache = self.tool.config.get_ldap_cache()
                ...
                self.closed = 0
                self.tool.logdebug(
                    f"Database opened (host={self.savedhost}, dbname={self.saveddbname}, user={self.saveduser})")
    ...
    def genUUID(self):
        ...
        """
        return md5.md5(f"{time.time()}-{random.random()}").hexdigest()
    ...
    def doSearch(self, key, fields=None, base="", scope=ldap.SCOPE_SUBTREE, flushcache=0):
        ...
        for tryit in range(3):
            try:
                ...
                if self.useldapcache and (not flushcache) and (scope == ldap.SCOPE_BASE) and self.ldapcache.has_key(
                        base):
                    entry = self.ldapcache[base]
                    self.tool.logdebug(f"LDAP cache hit {base} => {entry}")
                    result = [(base, entry)]
                else:
                    self.tool.logdebug(
                        f"QUERY : Filter : {key}, BaseDN : {base}, Scope : {scope}, Attributes : {fields}")
                    result = self.database.search_s(base, scope, key, fields)
            except ldap.NO_SUCH_OBJECT as msg:
                raise PyKotaStorageError(
                    f"Search base {base} doesn't seem to exist. Probable misconfiguration. Please double check /etc/pykota/pykota.conf : {msg}")
            except ldap.LDAPError as msg:
                message = (
                              f"Search for {key}({fields}) from {base}(scope={scope}) returned no answer.") + " : %s" % str(
                    msg)
                ...
                self.tool.logdebug(f"QUERY : Result : {result}")
                result = [(dn, cidict(attrs)) for (dn, attrs) in result]
                if self.useldapcache:
                    for (dn, attributes) in result:
                        self.tool.logdebug(f"LDAP cache store {dn} => {attributes}")
    ...
    def doAdd(self, dn, fields):
        ...
        for tryit in range(3):
            try:
                ...
                raise PyKotaStorageError(f"Entry {dn} already exists : {str(msg)}")
            except ldap.LDAPError as msg:
                message = f"Problem adding LDAP entry ({dn}, {str(fields)})  {str(msg)}"
                self.tool.printInfo(f"LDAP error : {message}", "error")
                self.tool.printInfo("LDAP connection will be closed and reopened.", "warn")
                self.close()
                self.secondStageInit()
            else:
                if self.useldapcache:
                    self.tool.logdebug(f"LDAP cache add {dn} => {fields}")
    ...
    def doDelete(self, dn):
        ...
        for tryit in range(3):
            try:
                self.tool.logdebug(f"QUERY : Delete({dn})")
                self.database.delete_s(dn)
            except ldap.NO_SUCH_OBJECT:
                self.tool.printInfo(f"Entry {dn} was already missing before we deleted it. This **MAY** be normal.",
                                    "info")
            except ldap.LDAPError as msg:
                message = f"Problem deleting LDAP entry ({dn})"
                f" : {str(msg)}"
                self.tool.printInfo(f"LDAP error : {message}", "error")
                self.tool.printInfo('LDAP connection will be closed and reopened.', "warn")
                self.close()
            ...
            try:
                ...
                if self.useldapcache and not flushcache:
                    if self.ldapcache.has_key(dn):
                        ...
                        self.tool.logdebug(f"LDAP cache hit {dn} => {old}")
                        oldentry = {}
                        for (k, v) in old.items():
                            if k != "createTimestamp":
                                oldentry[k] = v
                    else:
                        self.tool.logdebug(f"LDAP cache miss {dn}")
                        oldentry = self.doSearch("objectClass=*", base=dn, scope=ldap.SCOPE_BASE)[0][1]
                else:
                    oldentry = self.doSearch("objectClass=*", base=dn, scope=ldap.SCOPE_BASE, flushcache=flushcache)[0][
                        1]
                for (k, v) in fields.items():
                    if type(v) == type({}):
                        try:
                            ...
                            self.tool.logdebug(f"Error converting {oldentry.get(k)} with {k}({v})")
                            oldvalue = 0
                        if v["operator"] == '+':
                ...
                self.tool.logdebug(f"QUERY : Modify({dn}, {oldentry} ==> {fields})")
                ...
                self.tool.logdebug(f"MODIFY : {fields} ==> {entry} ==> {modentry}")
                if modentry:
                    self.database.modify_s(dn, modentry)
            except ldap.LDAPError as msg:
                message = (f"Problem modifying LDAP entry ({dn}, {fields})")
                f" : {str(msg)}"
                self.tool.printInfo(f"LDAP error : {message}", "error")
                self.tool.printInfo("LDAP connection will be closed and reopened.", "warn")
                self.close()
                    ...
                    for (mop, mtyp, mval) in entry:
                        if mop in (ldap.MOD_ADD, ldap.MOD_REPLACE):
                            ...
                            try:
                                ...
                                pass
                    self.tool.logdebug(f"LDAP cache update {dn} => {cachedentry}")
                return dn
        raise PyKotaStorageError(message)
    ...
    def filterNames(self, records, attribute, patterns=None):
        ...
        for (dn, record) in records:
            ...
            if attrval is None:
                self.tool.printInfo(f"Object {dn} has no {attribute} attribute !", "error")
            else:
    ...
    def getUserNbJobsFromHistory(self, user):
        ...
        result = self.doSearch(f"(&(pykotaUserName={self.userCharsetToDatabase(user.Name)})(objectClass=pykotaJob))",
                               None, base=self.info["jobbase"])
        return len(result)
    ...
    def getUserFromBackend(self, username):
        ...
        result = self.doSearch(
            f"(&(objectClass=pykotaAccount)(|(pykotaUserName={username})({self.info['userrdn']}={username})))",
            ["pykotaUserName", "pykotaLimitBy", self.info["usermail"], "description"], base=self.info["userbase"])
        if result:
            fields = result[0][1]
            ...
            result = self.doSearch(
                f"(&(objectClass=pykotaAccountBalance)(|(pykotaUserName={username})({self.info['balancerdn']}={username})))",
                ["pykotaBalance", "pykotaLifeTimePaid", "pykotaPayments", "pykotaOverCharge"],
                base=self.info["balancebase"])
            if not result:
                raise PyKotaStorageError(
                    f"No pykotaAccountBalance object found for user {username}. Did you create LDAP entries manually ?")
            else:
                fields = result[0][1]
    ...
    def getGroupFromBackend(self, groupname):
        ...
        result = self.doSearch(
            f"(&(objectClass=pykotaGroup)(|(pykotaGroupName={groupname})({self.info['grouprdn']}={groupname})))",
            ["pykotaGroupName", "pykotaLimitBy", "description"], base=self.info["groupbase"])
        if result:
            fields = result[0][1]
    ...
    def getUserPQuotaFromBackend(self, user, printer):
        ...
        if printer.Exists and user.Exists:
            ...
            result = self.doSearch(
                f"(&(objectClass=pykotaUserPQuota)(pykotaUserName={self.userCharsetToDatabase(user.Name)})(pykotaPrinterName={self.userCharsetToDatabase(printer.Name)}))",
                ["pykotaPageCounter", "pykotaLifePageCounter", "pykotaSoftLimit", "pykotaHardLimit", "pykotaDateLimit",
                 "pykotaWarnCount", "pykotaMaxJobSize"],
                base=base)
            if result:
                fields = result[0][1]
    ...
    def getGroupPQuotaFromBackend(self, group, printer):
        ...
        if group.Exists:
            ...
            result = self.doSearch(
                f"(&(objectClass=pykotaGroupPQuota)(pykotaGroupName={self.userCharsetToDatabase(group.Name)})(pykotaPrinterName={self.userCharsetToDatabase(printer.Name)}))", \
            ...
            if result:
                ...
                usernamesfilter = "".join([f"(pykotaUserName={self.userCharsetToDatabase(member.Name)})" for member in
                                           self.getGroupMembers(group)])
                if usernamesfilter:
                    usernamesfilter = f"(|{usernamesfilter})"
                if self.info["userquotabase"].lower() == "user":
                    base = self.info["userbase"]
                else:
                    base = self.info["userquotabase"]
                result = self.doSearch(
                    f"(&(objectClass=pykotaUserPQuota)(pykotaPrinterName={self.userCharsetToDatabase(printer.Name)}){usernamesfilter})", \
    ...
    def getPrinterLastJobFromBackend(self, printer):
        ...
        result = self.doSearch(
            f"(&(objectClass=pykotaLastjob)(|(pykotaPrinterName={pname})({self.info['printerrdn']}={pname})))",
            ["pykotaLastJobIdent"],
        ...
        if result:
            ...
            try:
                result = self.doSearch("objectClass=pykotaJob", ["pykotaJobSizeBytes",
                                                                 ...
                                                                 "createTimestamp"],
                                       base=f"cn={lastjobident},{self.info['jobbase']}", scope=ldap.SCOPE_BASE)
            ...
            if result:
                ...
                if lastjob.JobTitle == lastjob.JobFileName == lastjob.JobOptions == "hidden":
                    (lastjob.JobTitle, lastjob.JobFileName, lastjob.JobOptions) = (
                                                                                  "Hidden because of privacy concerns",) * 3
                date = fields.get("createTimestamp", ["19700101000000Z"])[0]  # It's in UTC !
                # mxtime = DateTime.strptime(date[:14], "%Y%m%d%H%M%S").localtime()
                mxtime = datetime.strptime(date[:14], "%Y%m%d%H%M%S").date()
    ...
    def getGroupMembersFromBackend(self, group):
        ...
        result = self.doSearch(
            f"(&(objectClass=pykotaGroup)(|(pykotaGroupName={gname})({self.info['grouprdn']}={gname})))",
    ...
    def getUserGroupsFromBackend(self, user):
        ...
        result = self.doSearch(f"(&(objectClass=pykotaGroup)({self.info['groupmembers']}={uname}))",
                               [self.info["grouprdn"], "pykotaGroupName", "pykotaLimitBy"],
                               base=self.info["groupbase"])
        if result:
            for (groupid, fields) in result:
                groupname = self.databaseToUserCharset(
                    (fields.get("pykotaGroupName", [None]) or fields.get(self.info["grouprdn"], [None]))[0])
    ...
    def getParentPrintersFromBackend(self, printer):
        ...
        pgroups = []
        result = self.doSearch(f"(&(objectClass=pykotaPrinter)(uniqueMember={printer.ident}))",
                               ["pykotaPrinterName"],
                               base=self.info["printerbase"])
        if result:
    ...
    def getMatchingPrinters(self, printerpattern):
        ...
        result = self.doSearch("objectClass=pykotaPrinter",
                               ["pykotaPrinterName", "pykotaPricePerPage", "pykotaPricePerJob", "pykotaMaxJobSize",
                                "pykotaPassThrough", "uniqueMember", "description"],
                               base=self.info["printerbase"])
        if result:
            ...
            try:
                ...
                for p in patterns:
                    patdict[p] = None
            for (printerid, fields) in result:
                printername = self.databaseToUserCharset(
                    fields.get("pykotaPrinterName", [""])[0] or fields.get(self.info["printerrdn"], [""])[0])
    ...
    def getMatchingUsers(self, userpattern):
        ...
        if result:
            ...
            for (userid, fields) in result:
                username = self.databaseToUserCharset(
                    fields.get("pykotaUserName", [""])[0] or fields.get(self.info["userrdn"], [""])[0])
                if patdict.has_key(username) or self.tool.matchString(username, patterns):
                    ...
                    result = self.doSearch(
                        f"(&(objectClass=pykotaAccountBalance)(|(pykotaUserName={uname})({self.info['balancerdn']}={uname})))",
                        ["pykotaBalance", "pykotaLifeTimePaid", "pykotaPayments", "pykotaOverCharge"],
                        base=self.info["balancebase"])
                    if not result:
                        raise PyKotaStorageError(
    ...
    def getMatchingGroups(self, grouppattern):
        ...
        if result:
            ...
            for (groupid, fields) in result:
                groupname = self.databaseToUserCharset(
                    fields.get("pykotaGroupName", [""])[0] or fields.get(self.info["grouprdn"], [""])[0])
    ...
    def getPrinterUsersAndQuotas(self, printer, names=["*"]):
        ...
        if self.info["userquotabase"].lower() == "user":
            ...
            base = self.info["userquotabase"]
        result = self.doSearch(
            f"(&(objectClass=pykotaUserPQuota)(pykotaPrinterName={pname})(|{''.join([f'(pykotaUserName={uname})' for uname in names])}))",
            ["pykotaUserName", "pykotaPageCounter", "pykotaLifePageCounter", "pykotaSoftLimit", "pykotaHardLimit",
             "pykotaDateLimit", "pykotaWarnCount"],
            base=base)
        if result:
        ...
        usersandquotas.sort(lambda x, y: self.cmp(x[0].Name, y[0].Name))
    ...
    def getPrinterGroupsAndQuotas(self, printer, names=["*"]):
        ...
        if self.info["groupquotabase"].lower() == "group":
            ...
            base = self.info["groupquotabase"]
        result = self.doSearch(
            f"(&(objectClass=pykotaGroupPQuota)(pykotaPrinterName={pname})(|{''.join([f'(pykotaGroupName={gname})' for gname in names])}))",
            ["pykotaGroupName"],
            base=base)
        if result:
        ...
        groupsandquotas.sort(lambda x, y: self.cmp(x[0].Name, y[0].Name))
    ...
    def addPrinter(self, printer):
        ...
        dn = f"{self.info['printerrdn']}={printername},{self.info['printerbase']}"
    ...
    def addUser(self, user):
        ...
        if self.info["newuser"].lower() != 'below':
            ...
            result = self.doSearch(f"(&(objectClass={where})({self.info['userrdn']}={uname}))",
                                   None,
                                   base=self.info["userbase"])
            if result:
                (dn, fields) = result[0]
                ...
                message = f"Unable to find an existing objectClass {where} entry with {self.info['userrdn']}={user.Name} to attach pykotaAccount objectClass"
                if action.lower() == "warn":
                    self.tool.printInfo(f"{message}. A new entry will be created instead.", "warn")
                else:  # 'fail' or incorrect setting
                    raise PyKotaStorageError(f"{message}. Action aborted. Please check your configuration.")
        ...
        if mustadd:
            ...
            fields.update(newfields)
            dn = f"{self.info['userrdn']}={uname},{self.info['userbase']}"
            self.doAdd(dn, fields)
            if self.info["userbase"] != self.info["balancebase"]:
                ...
                dn = f"{self.info['balancerdn']}={uname},{self.info['balancebase']}"
                self.doAdd(dn, fields)
        user.idbalance = dn
    ...
    def addGroup(self, group):
        ...
        if self.info["newgroup"].lower() != 'below':
            ...
            result = self.doSearch(f"(&(objectClass={where})({self.info['grouprdn']}={gname}))",
                                   None,
                                   base=self.info["groupbase"])
            if result:
                (dn, fields) = result[0]
                ...
                message = f"Unable to find an existing entry to attach pykotaGroup objectclass {group.Name}"
                if action.lower() == "warn":
                    self.tool.printInfo(f"{message}. A new entry will be created instead.", "warn")
                else:  # 'fail' or incorrect setting
                    raise PyKotaStorageError(f"{message}. Action aborted. Please check your configuration.")
    ...
    def addUserPQuota(self, upq):
        ...
        if self.info["userquotabase"].lower() == "user":
            dn = f"cn={uuid},{upq.User.ident}"
        else:
            dn = f"cn={uuid},{self.info['userquotabase']}"
        self.doAdd(dn, fields)
        upq.isDirty = False
        return None  # the entry created doesn't need further modification
    ...
    def addGroupPQuota(self, gpq):
        ...
        if self.info["groupquotabase"].lower() == "group":
            dn = f"cn={uuid},{gpq.Group.ident}"
        else:
            dn = f"cn={uuid},{self.info['groupquotabase']}"
        self.doAdd(dn, fields)
        gpq.isDirty = False
        return None  # the entry created doesn't need further modification
    ...
    def writeNewPayment(self, user, amount, comment=""):
        ...
        for payment in user.Payments:
            payments.append(
                f"{payment[0]} # {str(payment[1])} # {base64.encodestring(self.userCharsetToDatabase(payment[2])).strip()}")
        payments.append(
            f"{str(datetime.now())} # {str(amount)} # {base64.encodestring(self.userCharsetToDatabase(comment)).strip()}")
        fields = {
    ...
    def writeJobNew(self, printer, user, jobid, pagecounter, action, jobsize=None, jobprice=None, filename=None,
        ...
        if (not self.disablehistory) or (not printer.LastJob.Exists):
            uuid = self.genUUID()
            dn = f"cn={uuid},{self.info['jobbase']}"
        else:
        ...
        fields = {
            ...
            "pykotaBillingCode": self.userCharsetToDatabase(jobbilling),
            # don't add this attribute if it is not set, so no string conversion
        ...
        if printer.LastJob.Exists:
            ...
            lastjdn = f"cn={lastjuuid},{self.info['lastjobbase']}"
            fields = {
    ...
    def retrieveHistory(self, user=None, printer=None, hostname=None, billingcode=None, jobid=None, limit=100,
        ...
        where = []
        if user is not None:
            where.append(f"(pykotaUserName={self.userCharsetToDatabase(user.Name)})")
        if printer is not None:
            where.append(f"(pykotaPrinterName={self.userCharsetToDatabase(printer.Name)})")
        if hostname is not None:
            where.append(f"(pykotaHostName={hostname})")
        if billingcode is not None:
            where.append(f"(pykotaBillingCode={self.userCharsetToDatabase(billingcode)})")
        if jobid is not None:
            where.append(
                f"(pykotaJobId={jobid})")  # TODO : jobid is text, so self.userCharsetToDatabase(jobid) but do all of them as well.
        if where:
            where = f"(&{''.join([precond] + where)})"
        else:
            where = precond
        jobs = []
        ...
        if result:
            for (ident, fields) in result:
                ...
                if ((start is None) and (end is None)) or \
                    ...
                    job.Exists = True
                    jobs.append(job)
            jobs.sort(lambda x, y: self.cmp(y.JobDate, x.JobDate))
            if limit:
                jobs = jobs[:int(limit)]
        return jobs
    ...
    def deleteUser(self, user):
        ...
        uname = self.userCharsetToDatabase(user.Name)
        todelete = []
        result = self.doSearch(f"(&(objectClass=pykotaJob)(pykotaUserName={uname}))", base=self.info["jobbase"])
        for (ident, fields) in result:
        ...
        if self.info["userquotabase"].lower() == "user":
            ...
            base = self.info["userquotabase"]
        result = self.doSearch(f"(&(objectClass=pykotaUserPQuota)(pykotaUserName={uname}))",
                               ["pykotaPrinterName", "pykotaUserName"], base=base)
        for (ident, fields) in result:
        ...
        result = self.doSearch(f"(&(objectClass=pykotaAccountBalance)(pykotaUserName={uname}))",
                               ["pykotaUserName"],
                               base=self.info["balancebase"])
        for (ident, fields) in result:
            self.doDelete(ident)
    ...
    def deleteGroup(self, group):
        ...
        if self.info["groupquotabase"].lower() == "group":
            ...
            base = self.info["groupquotabase"]
        result = self.doSearch(f"(&(objectClass=pykotaGroupPQuota)(pykotaGroupName={gname}))",
                               ["pykotaGroupName"],
                               base=base)
        for (ident, fields) in result:
            self.doDelete(ident)
    ...
    def deleteUserPQuota(self, upquota):
        ...
        result = self.doSearch(f"(&(objectClass=pykotaJob)(pykotaUserName={uname})(pykotaPrinterName={pname}))",
                               base=self.info["jobbase"])
        for (ident, fields) in result:
            self.doDelete(ident)
    ...
    def deletePrinter(self, printer):
        ...
        pname = self.userCharsetToDatabase(printer.Name)
        result = self.doSearch(f"(&(objectClass=pykotaLastJob)(pykotaPrinterName={pname}))",
                               base=self.info["lastjobbase"])
        for (ident, fields) in result:
            self.doDelete(ident)
        result = self.doSearch(f"(&(objectClass=pykotaJob)(pykotaPrinterName={pname}))", base=self.info["jobbase"])
        for (ident, fields) in result:
            self.doDelete(ident)
        if self.info["groupquotabase"].lower() == "group":
            base = self.info["groupbase"]
            ...
            base = self.info["groupquotabase"]
        result = self.doSearch(f"(&(objectClass=pykotaGroupPQuota)(pykotaPrinterName={pname}))", base=base)
        for (ident, fields) in result:
            self.doDelete(ident)
        if self.info["userquotabase"].lower() == "user":
            base = self.info["userbase"]
            ...
            base = self.info["userquotabase"]
        result = self.doSearch(f"(&(objectClass=pykotaUserPQuota)(pykotaPrinterName={pname}))", base=base)
        for (ident, fields) in result:
            self.doDelete(ident)
        ...
        if entries:
            ...
            for entry in entries:
                result.append((entry.ident, entry.Name, entry.AccountBalance, entry.LifeTimePaid, entry.LimitBy,
                               entry.Email, entry.Description, entry.OverCharge))
            return [fields] + self.sortRecords(fields, result, ["+dn"], ordering)
    ...
    def extractBillingcodes(self, extractonly={}, ordering=[]):
        ...
        entries = [b for b in [self.getBillingCode(label) for label in self.getAllBillingCodes(billingcode)] if
                   b.Exists]
        if entries:
    ...
    def extractUpquotas(self, extractonly={}, ordering=[]):
        ...
        if entries:
            fields = (
            "username", "printername", "dn", "userdn", "printerdn", "lifepagecounter", "pagecounter", "softlimit",
            "hardlimit", "datelimit")
            result = []
            uname = extractonly.get("username")
            for entry in entries:
                for (user, userpquota) in self.getPrinterUsersAndQuotas(entry, names=[uname or "*"]):
                    result.append((user.Name, entry.Name, userpquota.ident, user.ident, entry.ident,
                                   userpquota.LifePageCounter, userpquota.PageCounter, userpquota.SoftLimit,
                                   userpquota.HardLimit, userpquota.DateLimit))
    ...
    def extractGpquotas(self, extractonly={}, ordering=[]):
        ...
        if entries:
            fields = (
            "groupname", "printername", "dn", "groupdn", "printerdn", "lifepagecounter", "pagecounter", "softlimit",
            "hardlimit", "datelimit")
            result = []
            gname = extractonly.get("groupname")
            for entry in entries:
                for (group, grouppquota) in self.getPrinterGroupsAndQuotas(entry, names=[gname or "*"]):
                    result.append((group.Name, entry.Name, grouppquota.ident, group.ident, entry.ident,
                                   grouppquota.LifePageCounter, grouppquota.PageCounter, grouppquota.SoftLimit,
                                   grouppquota.HardLimit, grouppquota.DateLimit))
    ...
    def extractHistory(self, extractonly={}, ordering=[]):
        ...
        enddate = extractonly.get("end")
        (startdate, enddate) = self.cleanDates(startdate, enddate)
        entries = self.retrieveHistory(user, printer, hostname=extractonly.get("hostname"),
                                       billingcode=extractonly.get("billingcode"), jobid=extractonly.get("jobid"),
                                       limit=None, start=startdate, end=enddate)
        if entries:
            fields = (
            "username", "printername", "dn", "jobid", "pagecounter", "jobsize", "action", "jobdate", "filename",
            "title", "copies", "options", "jobprice", "hostname", "jobsizebytes", "md5sum", "pages", "billingcode",
            "precomputedjobsize", "precomputedjobprice")
            result = []
            for entry in entries:
                result.append((entry.UserName, entry.PrinterName, entry.ident, entry.JobId, entry.PrinterPageCounter,
                               entry.JobSize, entry.JobAction, entry.JobDate, entry.JobFileName, entry.JobTitle,
                               entry.JobCopies, entry.JobOptions, entry.JobPrice, entry.JobHostName, entry.JobSizeBytes,
                               entry.JobMD5Sum, entry.JobPages, entry.JobBillingCode, entry.PrecomputedJobSize,
    ...
    def getBillingCodeFromBackend(self, label):
        ...
        result = self.doSearch(f"(&(objectClass=pykotaBilling)(pykotaBillingCode={ulabel}))",
    ...
    def addBillingCode(self, bcode):
        ...
        if oldentry.Exists:
            return oldentry  # we return the existing entry
        uuid = self.genUUID()
        dn = f"cn={uuid},{self.info['billingcodebase']}"
    ...
    def refundJob(self, jobident):
        ...
        dn = f"cn={jobident},{self.info['jobbase']}"
        fields = {
            "pykotaAction": "REFUND",
        }
        self.doModify(dn, fields)
