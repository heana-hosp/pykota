...
class StorageObject:
    ...
    def save(self):
        ...
        if self.isDirty:
            getattr(self.parent, f"save{self.__class__.__name__[7:]}")(self)
            self.isDirty = False
        ...
        if self.parent.usecache:
            for (k, v) in self.parent.caches["USERPQUOTAS"].items():
                if v.User.Name == self.Name:
                    self.parent.flushEntry("USERPQUOTAS", f"{v.User.Name}@{v.Printer.Name}")
        self.Exists = False
        self.isDirty = False
        ...
        if self.parent.usecache:
            for (k, v) in self.parent.caches["GROUPPQUOTAS"].items():
                if v.Group.Name == self.Name:
                    self.parent.flushEntry("GROUPPQUOTAS", f"{v.Group.Name}@{v.Printer.Name}")
        self.Exists = False
        self.isDirty = False
...
class StoragePrinter(StorageObject):
    ...
    def __getattr__(self, name):
        ...
        if name == "LastJob":
            ...
            self.parent.tool.logdebug(f"Lazy retrieval of last job for printer {self.Name}")
            return self.LastJob
        elif name == "Coefficients":
            self.Coefficients = self.parent.tool.config.get_printer_coefficients(self.Name)
            self.parent.tool.logdebug(
                f"Lazy retrieval of coefficients for printer {self.Name} : {self.Coefficients}")
            return self.Coefficients
        else:
            raise AttributeError(name)

    def addJobToHistory(self, jobid, user, pagecounter, action, jobsize=None, jobprice=None, filename=None, title=None,
                        copies=None, options=None, clienthost=None, jobsizebytes=None, jobmd5sum=None, jobpages=None,
                        jobbilling=None, precomputedsize=None, precomputedprice=None):
        """Adds a job to the printer's history."""
        self.parent.writeJobNew(self, user, jobid, pagecounter, action, jobsize, jobprice, filename, title, copies,
                                options, clienthost, jobsizebytes, jobmd5sum, jobpages, jobbilling, precomputedsize,
                                precomputedprice)
...
class StorageUserPQuota(StorageObject):
    ...
    def __getattr__(self, name):
        ...
        if name == "ParentPrintersUserPQuota":
            self.ParentPrintersUserPQuota = (
                                                        self.User.Exists and self.Printer.Exists and self.parent.getParentPrintersUserPQuota(
                                                    self)) or []
    ...
    def setDateLimit(self, datelimit):
        ...
        datelimit = datetime.fromisoformat(str(datelimit)[:19])
        date = f"{datelimit.year:04d}-{datelimit.month:02d}-{datelimit.day:02d} {datelimit.hour:02d}:{datelimit.minute:02d}:{datelimit.second:02d}"
    ...
    def setUsage(self, used):
        """Sets the PageCounter and LifePageCounter to used, or if used is + or - prefixed, changes the values of {
        Life,}PageCounter by that amount. """
        vused = int(used)
                    ...
                    if not inkusage:
                        ...
                        for pageindex in range(jobsize):
                            try:
                                ...
                                self.parent.tool.logdebug(
                                    f"No ink usage information. Using base cost of {pageprice:f} credits for page {pageindex + 1:d}.")
                                ...
                                for (ink, value) in usage.items():
                                    ...
                                    self.parent.tool.logdebug(
                                        f"Applying coefficient {coefvalue:f} for color {ink} (used at {value:f}% on "
                                        f"page {pageindex + 1:d}) to base cost {pageprice:f} gives {inkprice:f}")
                                    totalprice += inkprice
        if self.User.OverCharge != 1.0:  # TODO : beware of rounding errors
            overcharged = totalprice * self.User.OverCharge
            self.parent.tool.logdebug(
                f"Overcharging {totalprice} by a factor of {self.User.OverCharge} ===> User {self.User.Name} will be "
                f"charged for {overcharged} credits.")
            return overcharged
        else:
            return totalprice
...
class StorageGroupPQuota(StorageObject):
    ...
    def __getattr__(self, name):
        ...
        if name == "ParentPrintersGroupPQuota":
            self.ParentPrintersGroupPQuota = (
                                                     self.Group.Exists and self.Printer.Exists and self.parent.getParentPrintersGroupPQuota(
                                                 self)) or []
...
class StorageJob(StorageObject):
    ...
    def refund(self, reason):
        ...
        basereason = f"Refunded {self.JobSize:d} pages and {self.JobPrice:.3f} credits by {self.parent.tool.originalUserName} ({loginname}) on {str(datetime.now())[:19]}"
        if reason:
            reason = "%s : %s" % (basereason, reason)
        else:
            reason = basereason
        self.parent.tool.logdebug(f"Refunding job {self.ident}...")
        ...
        try:
            ...
            self.parent.tool.logdebug(f'Error while refunding job {self.ident}.')
            raise
        else:
            self.parent.commitTransaction()
            self.parent.tool.logdebug(f"Job {self.ident} refunded.")
...
class BaseStorage:
    def __init__(self, pykotatool):
        ...
        self.usecache = pykotatool.config.get_caching()
        self.disablehistory = pykotatool.config.get_disable_history()
        self.privacy = pykotatool.config.get_privacy()
        if self.privacy:
    ...
    def getFromCache(self, cachetype, key):
        ...
        if self.usecache:
            ...
            if entry is not None:
                self.tool.logdebug(f"Cache hit ({cachetype}->{key})")
            else:
                self.tool.logdebug(f"Cache miss ({cachetype}->{key})")
            return entry
    ...
    def cacheEntry(self, cachetype, key, value):
        ...
        if self.usecache and getattr(value, "Exists", 0):
            ...
            self.tool.logdebug(f"Cache store ({cachetype}->{key})")
    ...
    def flushEntry(self, cachetype, key):
        ...
        if self.usecache:
            try:
                ...
                self.tool.logdebug(f"Cache flush ({cachetype}->{key})")
    ...
    def getUserPQuota(self, user, printer):
        ...
        useratprinter = f"{user.Name}@{printer.Name}"
    ...
    def getParentPrinters(self, printer):
        ...
        if self.usecache:
            if not hasattr(printer, "Parents"):
                self.tool.logdebug(f"Cache miss ({printer.Name}->Parents)")
                printer.Parents = self.getParentPrintersFromBackend(printer)
                self.tool.logdebug(f"Cache store ({printer.Name}->Parents)")
            else:
                self.tool.logdebug(f"Cache hit ({printer.Name}->Parents)")
        else:
            printer.Parents = self.getParentPrintersFromBackend(printer)
    ...
    def getGroupMembers(self, group):
        ...
        if self.usecache:
            if not hasattr(group, "Members"):
                self.tool.logdebug(f"Cache miss ({group.Name}->Members)")
                group.Members = self.getGroupMembersFromBackend(group)
                self.tool.logdebug(f"Cache store ({group.Name}->Members)")
            else:
                self.tool.logdebug(f"Cache hit ({group.Name}->Members)")
        else:
            group.Members = self.getGroupMembersFromBackend(group)
    ...
    def getUserGroups(self, user):
        ...
        if self.usecache:
            if not hasattr(user, "Groups"):
                self.tool.logdebug(f"Cache miss ({user.Name}->Groups)")
                user.Groups = self.getUserGroupsFromBackend(user)
                self.tool.logdebug(f"Cache store ({user.Name}->Groups)")
            else:
                self.tool.logdebug(f"Cache hit ({user.Name}->Groups)")
        else:
            user.Groups = self.getUserGroupsFromBackend(user)
        return user.Groups
    ...
    def cleanDates(self, startdate, enddate):
        ...
        for limit in datedict.keys():
            ...
            if dateval:
                ...
                if not dateval.isdigit():
                    ...
                    if lgdateval == 4:
                        if limit == "start":
                            dateval = f"{dateval}0101 00:00:00"
                        else:
                            dateval = f"{dateval}1231 23:59:59"
                    elif lgdateval == 6:
                        if limit == "start":
                            dateval = f"{dateval}01 00:00:00"
                        else:
                            # mxdate = DateTime.ISO.ParseDateTime("%s01 00:00:00" % dateval)
                            mxdate = datetime.fromisoformat(f"{dateval}01 00:00:00")
                            dateval = f"{dateval}{mxdate.days_in_month:02d} 23:59:59"
                    elif lgdateval == 8:
                        if limit == "start":
                            dateval = f"{dateval} 00:00:00"
                        else:
                            dateval = f"{dateval} 23:59:59"
                    elif lgdateval == 10:
                        if limit == "start":
                            dateval = f"{dateval[:8]} {dateval[8:]}:00:00"
                        else:
                            dateval = f"{dateval[:8]} {dateval[8:]}:59:59"
                    elif lgdateval == 12:
                        if limit == "start":
                            dateval = f"{dateval[:8]} {dateval[8:10]}:{dateval[10:]}:00"
                        else:
                            dateval = f"{dateval[:8]} {dateval[8:10]}:{dateval[10:]}:59"
                    elif lgdateval == 14:
                        dateval = f"{dateval[:8]} {dateval[8:10]}:{dateval[10:12]}:{dateval[12:]}"
                    else:
                        dateval = None
                    try:
                        # DateTime.ISO.ParseDateTime(dateval[:19])
                        datetime.fromisoformat(dateval[:19])
                    except:
                        dateval = None
        ...
        try:
            if len(start) == 17:
                start = f"{start[0:4]}-{start[4:6]}-{start[6:8]} {start[9:]}"
        except TypeError:
            pass

        try:
            if len(end) == 17:
                end = f"{end[0:4]}-{end[4:6]}-{end[6:8]} {end[9:]}"
        except TypeError:
            pass
...
def openConnection(pykotatool: object) -> object:
    """Returns a connection handle to the appropriate database."""
    backendinfo = pykotatool.config.get_storage_backend()
    ...
    try:
        ...
        #                                              "storages",
        #                                              "%s.py" % backend.lower()))
        storagebackend = SourceFileLoader("storagebackend",
                                          f"{os.path.join(os.path.dirname(__file__))}/storages/{backend.lower()}.py").load_module()
        #sys.stdout.write('moduco importado')
    except ImportError:
        raise PyKotaStorageError(f"Unsupported quota storage backend {backend}")
    else:
        ...
        #sys.stderr.write(f"({pykotatool.config.get_storage_backend},{host}, {database}, {admin}, {adminpw})")
