...
class BaseHandler:
    ...
    def checkIfError(self, errorstates):
        ...
        if errorstates is None:
            ...
            try:
                errormask = self.parent.filter.config.get_printer_snmp_error_mask(self.parent.filter.PrinterName)
            ...
            self.parent.filter.logdebug(f"Error conditions for mask 0x{errormask:04x} : {errorConditions}")
            for err in errorstates:
                if err in errorConditions:
                    self.parent.filter.logdebug(
                        f"Error condition '{err}' encountered. PyKota will wait until this problem is fixed.")
                    return True
            self.parent.filter.logdebug(f"No error condition matching mask 0x{errormask:04x}")
            return False
    ...
    def waitPrinting(self):
        ...
        if not noprintingmaxdelay:
            self.parent.filter.logdebug(
                f"Will wait indefinitely until printer {self.parent.filter.PrinterName} is in 'printing' state.")
        else:
            self.parent.filter.logdebug(
                f"Will wait until printer {self.parent.filter.PrinterName} is in 'printing' state or {noprintingmaxdelay:d} seconds have elapsed.")
        previousValue = self.parent.get_last_page_counter()
        firstvalue = None
        while 1:
            ...
            if self.printerInternalPageCounter is not None:
                if firstvalue is None:
                    ...
                    if firstvalue < self.printerInternalPageCounter:
                        ...
                        self.parent.filter.printInfo(f"Printer {self.parent.filter.PrinterName} is lying to us !!!",
                                                     "warn")
                        break
                        ...
                        if (pstatusAsString == 'idle') or \
                            ...
                            if self.printerInternalPageCounter == previousValue:
                                ...
                                self.parent.filter.printInfo(
                                    f"Printer {self.parent.filter.PrinterName} probably won't print this job !!!",
                                    "warn")
                            else:
                                # Here the job has already been entirely printed, and
                                # the printer has already passed from 'idle' to 'printing' to 'idle' again.
                                self.parent.filter.printInfo(
                                    f"Printer {self.parent.filter.PrinterName} has probably already printed this job !!!",
                                    "warn")
                            break
            self.parent.filter.logdebug(f"Waiting for printer {self.parent.filter.PrinterName} to be printing...")
    ...
    def waitIdle(self):
        ...
        while 1:
            ...
            if idle_flag:
                ...
                idle_num = 0
            self.parent.filter.logdebug(
                f"Waiting for printer {self.parent.filter.PrinterName}'s idle status to stabilize...")
            time.sleep(statusstabilizationdelay)
    ...
    def retrieveInternalPageCounter(self):
        ...
        try:
            ...
            self.parent.filter.printInfo(
                f"SNMP querying stage interrupted. Using latest value seen for internal page counter ({self.printerInternalPageCounter}) on printer {self.parent.filter.PrinterName}.",
                "warn")
            raise
...
if hasV4:
    class Handler(BaseHandler):
        ...
        def retrieveSNMPValues(self):
            ...
            if errorIndication:
                self.parent.filter.printInfo(f"SNMP Error : {errorIndication}", "error")
                self.initValues()
            elif errorStatus:
                self.parent.filter.printInfo(
                    f"SNMP Error : {errorStatus.prettyPrint()} at {varBinds[int(errorIndex) - 1]}", "error")
                self.initValues()
            else:
                self.printerInternalPageCounter = max(self.printerInternalPageCounter,
                                                      int(varBinds[0][1].prettyPrint() or "0"))
                ...
                self.parent.filter.logdebug(
                    f"SNMP answer decoded : PageCounter : {self.printerInternalPageCounter}  PrinterStatus : '{printerStatusValues.get(self.printerStatus)}'  DeviceStatus : '{deviceStatusValues.get(self.deviceStatus)}'  PrinterErrorState : '{self.printerDetectedErrorState}'")
else:
    class Handler(BaseHandler):
        ...
        def retrieveSNMPValues(self):
            ...
            except (SnmpOverUdpError, select.error) as msg:
                self.parent.filter.printInfo(
                    f"Network error while doing SNMP queries on printer {self.printerHostname} : {msg}", "warn")
                self.initValues()
            tsp.close()
        ...
        def handleAnswer(self, wholeMsg, notusedhere, req):
            ...
            try:
                ...
                self.parent.filter.printInfo(f"SNMP message decoding error for printer {self.printerHostname} : {msg}",
                                             "warn")
                self.initValues()
            else:
                if req.apiAlphaMatch(rsp):
                    ...
                    if errorStatus:
                        self.parent.filter.printInfo(
                            f"Problem encountered while doing SNMP queries on printer {self.printerHostname} : {errorStatus}",
                            "warn")
                    else:
                        self.values = []
                        ...
                        try:
                            ...
                            self.parent.filter.logdebug(f"SNMP answer is incomplete : {str(self.values)}")
                            pass
                        else:
                            return 1
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.stderr.write(f"Usage :  python  {sys.argv[0]}  printer_ip_address\n")
    else:
        def _(msg):
            return msg
        ...
        print(f"Internal page counter's value is : {pagecounter}")
