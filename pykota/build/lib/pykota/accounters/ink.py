...
class Accounter(AccounterBase):
    ...
    def computeJobSize(self):
        ...
        if (not self.isPreAccounter) and \
            ...
            # know the result.
            self.filter.logdebug(f"Precomputing pass told us that job is {self.filter.softwareJobSize} pages long.")
        ...
        if colorspace not in ("cmyk", "bw", "cmy", "rgb", "gc"):
            raise PyKotaAccounterError(f"Invalid parameters for ink accounter : [{self.arguments}]")
        ...
        if self.filter.JobSizeBytes:
            try:
                ...
                self.filter.printInfo(
                    "pkpgcounter is now distributed separately, please grab it from "
                ...
                try:
                    ...
                    # job's size MAY be.
                    self.filter.printInfo(
                        f"Unable to precompute the job's size and ink coverage with the generic PDL analyzer : {msg}",
                        "warn")
                else:
